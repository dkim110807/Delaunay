//
// Created by dkim110807 on 2023-12-21.
//
...
// Credit https://github.com/delfrrr/delaunator-cpp/blob/master/include/delaunator.hpp
// Some comments by dkim110807
namespace delaunator {
    inline size_t fast_mod(const std::size_t i, const std::size_t c) {
        return i >= c ? i % c : i;
    }

    inline double sum(const std::vector<double> &x) {
        double sum = x[0], err = 0.0;
    ...
    // (ax, ay), (bx, by), (cx, cy) circum circle radius
    // returns square of radius
    inline double circumradius(const double ax, const double ay, const double bx, const double by, const double cx, const double cy) {
        const double dx = bx - ax, dy = by - ay, ex = cx - ax, ey = cy - ay;
        const double bl = dx * dx + dy * dy, cl = ex * ex + ey * ey, d = dx * ey - dy * ex;
        const double x = (ey * bl - dy * cl) * 0.5 / d, y = (dx * cl - ex * bl) * 0.5 / d;
    ...
    // returns center of circum circle
    inline std::pair<double, double> circumcenter(const double ax, const double ay, const double bx, const double by, const double cx, const double cy) {
        const double dx = bx - ax, dy = by - ay, ex = cx - ax, ey = cy - ay;
        const double bl = dx * dx + dy * dy, cl = ex * ex + ey * ey, d = dx * ey - dy * ex;
        const double x = ax + (ey * bl - dy * cl) * 0.5 / d, y = ay + (dx * cl - ex * bl) * 0.5 / d;
        return {x, y};
    }
    ...
    // https://computergraphics.stackexchange.com/questions/10522/understand-a-pseudo-angle-implementation
    ...
    private:
        ...
        std::size_t legalize(std::size_t a) {
            ...
            while (true) {
                ...
                /*
                 * If the pair of triangles doesn't satisfy the Delaunay condition => flip
                 * Same checkk/flip recursively for the new pair of triangles
                 *
                 *        pl                    pl
                 *       /||\                  /  \
                 *    al/ || \bl            al/    \a
                ...
                // Delaunay triangulation should not have other points in circum circle
        ...
        [[nodiscard]] std::size_t hash_key(double x, double y) const {
            const double dx = x - m_center_x, dy = y - m_center_y;
    ...
    public:
        ...
        std::vector<std::size_t> triangles, halfedges, hull_prev, hull_next, hull_tri;
        std::size_t hull_start;

        // https://www.s-hull.org/paper/s_hull.pdf
        explicit Delaunator(const std::vector<double> &in_coords) : coords(in_coords) {
            ...
            for (std::size_t i = 0; i < n; i++) {
                const double x = coords[2 * i], y = coords[2 * i + 1];
                if (x < min_x) min_x = x;
                if (y < min_y) min_y = y;
                if (x > max_x) max_x = x;
                if (y > max_y) max_y = y;
            ...
            const double cx = (min_x + max_x) / 2, cy = (min_y + max_y) / 2;
            double min_dist = std::numeric_limits<double>::max();

            std::size_t i0 = INVALID_INDEX, i1 = INVALID_INDEX, i2 = INVALID_INDEX;

            // Pick a seed point close to the centroid (center of points)
            for (std::size_t i = 0; i < n; i++) {
                const double d = dist(cx, cy, coords[2 * i], coords[2 * i + 1]);
                if (d < min_dist) i0 = i, min_dist = d;
            }

            const double i0x = coords[2 * i0], i0y = coords[2 * i0 + 1];

            min_dist = std::numeric_limits<double>::max();

            // Find the point closest to the seed (S-Hull Algorithm Step 2)
            for (std::size_t i = 0; i < n; i++) {
                if (i == i0) continue;
                const double d = dist(i0x, i0y, coords[2 * i], coords[2 * i + 1]);
                if (d < min_dist && d > 0.0) i1 = i, min_dist = d;
            }
            ...
            // Find the third point which forms the smallest circum circle with the first two points
            for (std::size_t i = 0; i < n; i++) {
            ...
            // If counter clock wise => clock wise
            if (orient(i0x, i0y, i1x, i1y, i2x, i2y)) {
                std::swap(i1, i2), std::swap(i1x, i2x), std::swap(i1y, i2y);
            }

            // Find the circum center of i0, i1, i2
            ...
            m_hash.resize(m_hash_size, INVALID_INDEX);
            ...
            triangles.reserve(max_triangles * 3), halfedges.reserve(max_triangles * 3);
            add_triangle(i0, i1, i2, INVALID_INDEX, INVALID_INDEX, INVALID_INDEX);
            double xp = std::numeric_limits<double>::quiet_NaN(), yp = std::numeric_limits<double>::quiet_NaN();
            for (std::size_t k = 0; k < n; k++) {
                ...
                if (e == start) {
                    while (q = hull_prev[e], orient(x, y, coords[2 * q], coords[2 * q + 1], coords[2 * e], coords[2 * e + 1])) {
                        ...
                        legalize(t + 2);
                        hull_tri[q] = t, hull_next[e] = e; // mark as removed
                        hull_size--;
                        e = q;
                    }
                }
                ...
                hull_prev[i] = hull_start = e;
                hull_prev[next] = hull_next[e] = i;
                hull_next[i] = next;
